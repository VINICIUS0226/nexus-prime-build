...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  const {
    ...
    readNotificationIds,
    markAsRead,
  } = useSystemNotifications({ enabled: notificationsOpen, limit: 10 });

  useEffect(() => {
    if (!notificationsOpen) return;
    if (notificationsLoading) return;
    if (!notifications.length) return;
    const unreadIds = notifications.filter((n) => !readNotificationIds.has(n.id)).map((n) => n.id);
    if (!unreadIds.length) return;
    markAsRead(unreadIds);
  }, [markAsRead, notifications, notificationsLoading, notificationsOpen, readNotificationIds]);
      ...
      <Popover open={notificationsOpen} onOpenChange={setNotificationsOpen}>
        ...
        <PopoverContent align="end" className="w-80">
          ...
          {notificationsLoading ? (
            <div className="py-6 px-4 text-sm text-muted-foreground">Carregando...</div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className={`text-sm font-semibold ${isUnread ? 'text-primary' : 'text-foreground'}`}>
                            {n.title}
                          </p>
                      ...
                      <p className="text-xs text-muted-foreground mt-2 line-clamp-3">{n.message}</p>
                    </div>
                  );
                })}
              </div>
